...
export class GrantsRepository {
  ...
  async getGrants(filters?: {
    ...
    if (filters?.query) {
      const sanitizedTerm = sanitizeQueryTerm(filters.query);
      if (sanitizedTerm) {
        query = query.or(
          `title.ilike.%${sanitizedTerm}%,agency.ilike.%${sanitizedTerm}%,summary.ilike.%${sanitizedTerm}%,synopsis_desc.ilike.%${sanitizedTerm}%`
        );
      }
    }
